// components/NewProgrammedSavingForm.tsx
...
const NewProgrammedSavingForm: React.FC = () => {
    const [formData, setFormData] = useState<Omit<ProgrammedSaving, 'numeroCartola' | 'clienteId' | 'saldoActual' | 'estadoPlan' | 'fechaCreacion' | 'ultimaActualizacion'>>({
        ...
        adminCreadorId: 'sistema', // O el ID del admin si es relevante
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!currentUser) {
            setError("Debes estar autenticado para crear un plan.");
            return;
        }

        if (formData.montoMeta <= 0) {
            setError("El monto meta debe ser mayor a cero.");
            return;
        }
        ...
        try {
            await createProgrammedSaving(currentUser.uid, formData);
            navigate('/portal/ahorros'); // Redirige a la lista de ahorros
        } catch (err) {
            setError("Error al crear el plan de ahorro. Por favor, inténtelo de nuevo.");
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="container mx-auto p-4 md:p-8 max-w-2xl">
            <div className="bg-white p-8 rounded-xl shadow-lg">
                <h1 className="text-3xl font-bold text-gray-800 mb-6">Crear Nuevo Plan de Ahorro</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        ...
                        <input
                            ...
                            placeholder='Ej: Ahorro para mi casa'
                    ...
                    <div className="flex justify-end pt-4">
                        <button 
                            type="button" 
                            onClick={() => navigate('/portal/ahorros')} 
...
export default NewProgrammedSavingForm;
